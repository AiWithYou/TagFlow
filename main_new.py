...
# -*- coding: utf-8 -*-
"""
リファクタリング済みの画像タグ付け・ファイル移動ツール
・AIサーバー停止機能を削除
・削除パターン設定のGUIを強化（パターンテスト機能追加）
・全体的にコードを整理し可読性・保守性向上
・AIチャットで最新メッセージが常に見えるように修正
"""
...
# PySide6インポート
from PySide6.QtGui import (QAction, QPixmap, QIcon, QImage, QColor, QPalette,
                           QFont, QTextCursor)
from PySide6.QtWidgets import (
    ...
    QGroupBox, QRadioButton, QStatusBar, QToolBar, QDialog, QMessageBox, QGridLayout
)
...
class ImageAnalyzer:
    ...
    def __init__(
        ...
        # HEIC対応のため、'.heic'を追加
        self.supported_formats = {'.jpg', '.jpeg', '.png', '.gif', '.bmp', '.webp', '.heic'}
    ...
    def get_prompt(self):
        """
        カスタムプロンプトがあればそれを使用し、なければdetail_levelとuse_japaneseに応じたデフォルトを返す
        """
    ...
    def clean_response_text(self, response):
        """
        画像から得られたレスポンステキストを削除パターンに基づいてクリーンアップする
        """
        return apply_clean_patterns(response, self.clean_patterns)

    def analyze_image(self, image_path):
        """
        1枚の画像をAPIに送信して分析し、テキスト（タグ）を返す
        """
        try:
...
class FileSearchWorker(QThread):
    """
    テキストファイル内のタグ検索を別スレッドで実行するためのワーカー
    """
    progress_updated = Signal(float)
...
class FileOperationWorker(QThread):
    """
    ファイルのコピー・移動処理を別スレッドで実行するためのワーカー
    """
    progress_updated = Signal(float)
...
class PatternTestDialog(QDialog):
    """
    ユーザーが入力した削除パターンがサンプルテキストに対してどのように作用するかをテストするためのダイアログ
    """
...
class ChatTab(QWidget):
    ...
    def __init__(self, parent=None, get_api_url_func=None, get_model_func=None):
        ...
        self.input_edit.installEventFilter(self)  # Shift+Enter送信用
...
class FileMoveTab(QWidget):
    ...
    def search_files(self):
        ...
        self.search_worker = FileSearchWorker(self.source_edit.text(), search_terms, search_mode, use_regex)
    ...
    def process_files(self, operation):
        ...
        self.operation_worker.operation_complete.connect(lambda count: self.on_operation_complete(count, operation))
...
class ResultEditTab(QWidget):
    ...
    def select_folder(self):
        """
        編集対象フォルダを選択し、画像と対応するtxtファイルを一覧にロード
        """
    ...
    def run_bulk_edit(self):
        """
        カレントフォルダ内の全txtに対してまとめて編集操作（削除/先頭末尾追記/文字列置換）を行う
        """
...
class MainWindow(QMainWindow):
    ...
    def start_ai_server(self):
        """
        ollama serve を起動する
        Windowsの場合は新しいコンソールで起動し、macOS/Linuxではバックグラウンドで起動。
        """
    ...
    def set_light_theme(self):
        ...
        self.setStyleSheet("""
            ...
            QLabel { color: #000000; font-size: 11pt; }
            QLineEdit, QTextEdit, QListWidget, QComboBox {
                background-color: #ffffff; color: #000000;
                border: 1px solid #cccccc; border-radius: 3px; padding: 4px;
            }
            QPushButton {
                background-color: #f0f0f0; color: #000000; font-size: 10pt;
                border: 1px solid #cccccc; border-radius: 4px; padding: 8px 16px;
            }
            ...
            QTabBar::tab {
                ...
                border: 1px solid #cccccc; margin-right: 2px;
            }
            QTabBar::tab:selected {
                background-color: #e0e0e0; border-bottom-color: #e0e0e0;
            }
            QTabBar::tab:hover { background-color: #e0e0e0; }
            QGroupBox {
                border: 1px solid #cccccc; border-radius: 5px;
                margin-top: 2ex; padding-top: 5px; font-weight: bold; color: #000000;
            }
            QProgressBar {
                background-color: #f0f0f0; color: #000000;
                border: 1px solid #cccccc; border-radius: 3px; text-align: center;
            }
