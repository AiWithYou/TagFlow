...
- 変換モードを選ぶ。
- 使用モデルを選ぶ。候補外の Ollama モデル名も手入力できる。
- API URL を確認する。初期値は `http://localhost:11434/api/generate`。
...
`image.tagflow.json` には変換履歴が保存されます。履歴には変換モード、入力ファイル、出力ファイル、使用モデル、API URL、作成日時、入力と出力の SHA-256 ハッシュが入ります。既存の `.tagflow.json` がある場合は `transforms` 配列に履歴を追記します。壊れた JSON がある場合は `.bak.json` として退避し、新しい履歴ファイルを作成します。
...
6. `変換モード` を選ぶ。
7. `使用モデル` を選ぶ。候補外のモデル名も直接入力できる。
8. `API URL` が `http://localhost:11434/api/generate` になっていることを確認する。
9. 一覧から画像を選び、変換前テキストを確認する。
10. まず `選択ファイルをプレビュー変換` で 1 件だけ結果を見る。
11. 問題なければ `一括変換` を押す。
12. 処理中に止めたい場合は `停止` を押す。
...
テキスト変換機能は `checklists/acceptance_tests_ja.md` を確認してください。起動、既存 4 タブの表示、新規タブの表示、プレビュー変換、一括変換、サイドカー保存、`.tagflow.json` 履歴、Ollama 未起動時のエラー表示、停止ボタンの動作を確認します。
